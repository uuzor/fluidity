...
async function main() {
  ...
  try {
    ...
    // Deploy SecureStabilityPool - will be deployed after we have all required addresses
    let stabilityPool;

    // Step 5: Deploy Core Protocols
    console.log("\n=== Deploying Core Protocol Contracts ===");

    // Deploy SecureTroveManager with library linking
    ...
    // Deploy SecureBorrowerOperations
    ...
    // Now deploy SecureStabilityPool with all required constructor parameters
    const SecureStabilityPool = await ethers.getContractFactory("SecureStabilityPool");
    stabilityPool = await SecureStabilityPool.deploy(
    ...
    // SecureStabilityPool is initialized via constructor - no additional setup needed
    console.log("✅ StabilityPool already initialized via constructor");
    ...
    const filename = `deployments-${deploymentData.network.chainId}.json`;
    fs.writeFileSync(filename, JSON.stringify(deploymentData, null, 2));
    console.log(`\n💾 Deployment addresses saved to ${filename}`);

    console.log("\n📝 === NEXT STEPS ===");
    console.log("1. Verify contracts on block explorer");
    console.log("2. Set up governance parameters");
    console.log("3. Add additional price oracles for other assets");
    console.log("4. Configure liquidation parameters");
