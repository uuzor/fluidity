...
/**
 * @title TroveManagerV2
 * @notice V2: Single source of truth for trove data with proper BorrowerOperations integration
 * @dev Complete redesign with clear separation of concerns
 *
 * Architecture Changes from V1:
 * - ✅ ONLY contract that stores trove data (_packedTroves)
 * - ✅ BorrowerOperationsV2 calls updateTrove/closeTrove for all state changes
 * - ✅ Liquidation functions read and modify the single source of truth
 * - ✅ All our closeTrove fixes are now actively used
 * - ✅ Proper access control: only BorrowerOps can modify troves
 *
 ...
 *
 * Fixes Applied:
 * 1. ✅ Removed unused 'trove' variable in closeTrove
 * 2. ✅ Added sortedTroves.remove() in closeTrove
 * 3. ✅ Added TroveUpdated event emission in closeTrove
 *
 * @custom:security-contact security@fluidprotocol.com
 */
contract TroveManagerV2 is OptimizedSecurityBase, ITroveManager {
    ...
    /// @notice Collateral scaling factor (PackedTrove stores in reduced precision)
    ...
    /// @dev THIS IS THE ONLY TROVE STORAGE - BorrowerOpsV2 does NOT duplicate this
    ...
    function liquidate(
        ...
        // Get trove data from OUR storage (single source of truth)
        ...
        vars.collateral = uint256(trove.collateral) * COLL_SCALE; // Scale back
    ...
    function batchLiquidateTroves(
        ...
        for (uint256 i = 0; i < iterations;) {
            ...
            if (trove.status == PackedTrove.STATUS_ACTIVE) {
                vars.debt = uint256(trove.debt);
                vars.collateral = uint256(trove.collateral) * COLL_SCALE;
                vars.icr = _calculateICR(vars.collateral, vars.debt, vars.price);
    ...
    function liquidateTroves(
        ...
        for (uint256 i = 0; i < n && currentBorrower != address(0);) {
            ...
            if (trove.status == PackedTrove.STATUS_ACTIVE) {
                ...
                uint256 collateral = uint256(trove.collateral) * COLL_SCALE;
    ...
    // V2 CHANGE: These methods are now ACTIVELY CALLED by BorrowerOperationsV2

    /**
     * @notice Update trove data (called by BorrowerOperationsV2)
     * @dev V2: This is now the ONLY way to update trove data
     */
    function updateTrove(
        ...
        // Update trove data in OUR storage (single source of truth)
        _packedTroves[borrower][asset] = PackedTrove.pack(
            uint128(debt),
            collateral,
            uint32(block.timestamp),
    ...
    /**
     * @notice Remove stake (called by BorrowerOperationsV2)
     */
    ...
    /**
     * @notice Close trove (called by BorrowerOperationsV2)
     * @dev V2: Now actively used! All our fixes apply here:
     *      1. ✅ No unused 'trove' variable
     *      2. ✅ sortedTroves.remove() called
     *      3. ✅ TroveUpdated event emitted
     */
    function closeTrove(address borrower, address asset) external override {
        ...
        // Update trove status (FIX #1: No unused variable unpack)
        _packedTroves[borrower][asset] = PackedTrove.pack(
            0,
            0,
            uint32(block.timestamp),
        ...
        // FIX #2: Remove from sorted troves
        sortedTroves.remove(asset, borrower);

        // FIX #3: Emit TroveUpdated event (operation = 2 for closeTrove)
    ...
    // ============ View Functions ============
    // V2: These read from OUR storage (single source of truth)
    ...
    function getTroveDebtAndColl(
        ...
        return (
            uint256(trove.debt),
            uint256(trove.collateral) * COLL_SCALE
        );
    }
    ...
    function getEntireDebtAndColl(
        ...
        collateral = uint256(trove.collateral) * COLL_SCALE + pendingCollateralReward;
    }
    ...
    function _applyPendingRewards(address borrower, address asset) internal {
        ...
        uint256 newColl = (uint256(trove.collateral) * COLL_SCALE) + pendingCollReward;
