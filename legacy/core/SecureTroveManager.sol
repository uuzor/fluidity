...
contract SecureTroveManager is SecurityBase, ITroveManager {
    ...
    // Events are inherited from ITroveManager interface
        ...
        returns (uint256, uint256) 
    {
        ...
        // Input validation - allow zero address for ETH
        _validateAddress(borrower);
        // Skip asset validation for ETH (zero address)
        ...
        // Initialize global totals if this is the first trove
        if (totalDebt[asset] == 0 && totalCollateral[asset] == 0) {
            totalDebt[asset] = debt;
            totalCollateral[asset] = coll;
            totalStakes[asset] = coll; // Initial stake equals collateral
        }
    ...
    function registerForSonicFeeM(uint256 projectId) external onlyValidRole(accessControl.ADMIN_ROLE()) {
        ...
        emit SecurityCheck(msg.sender, "feem_registration", true, "Contract registered for Sonic FeeM");
    }
}
